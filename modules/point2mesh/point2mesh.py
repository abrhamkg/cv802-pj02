...
from modules.point2mesh.models.layers.mesh import Mesh, PartMesh
from modules.point2mesh.models.networks import init_net, sample_surface, local_nonuniform_penalty
import modules.point2mesh.utils as utils
import numpy as np
from modules.point2mesh.models.losses import chamfer_distance, BeamGapLoss
from modules.point2mesh.options import Options
import time
import os
...
def run_point2mesh(initial_mesh, input_pc):
    device = torch.device('cuda:{}'.format(opts.gpu) if torch.cuda.is_available() else torch.device('cpu'))
    print('device: {}'.format(device))

    # initial mesh
    mesh = Mesh(initial_mesh, device=device, hold_history=True)
    # mesh = mesh.to(device)
    # input point cloud
    input_xyz, input_normals = utils.read_pts(input_pc)
